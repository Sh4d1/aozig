...
pub fn solve(input: []i32, last: bool) ![]i32 {
    var l = input;
    var single = std.array_list.AlignedManaged(i32, null).init(alloc);
    if (last) {
    ...
    while (true) {
        var new_line = std.array_list.AlignedManaged(i32, null).init(alloc);
        for (l[1..], 1..) |_, i| {
...
pub fn parse(input: []const u8) ![][]i32 {
    var res = std.array_list.AlignedManaged([]i32, null).init(alloc);
    var lines = std.mem.tokenizeScalar(u8, input, '\n');

    while (lines.next()) |l| {
        var split = std.mem.tokenizeScalar(u8, l, ' ');
        var line = std.array_list.AlignedManaged(i32, null).init(alloc);
        while (split.next()) |d| {
...
test "test-1" {
    const res = try solve1(try parse(test_data));
    try std.testing.expectEqual(res, 114);
}

test "test-2" {
    const res = try solve2(try parse(test_data));
